...
import type { QuizImageSlide as Model } from '../types/manifest'

interface Props {
  slide: Model
}

export function QuizImageSlide({ slide }: Props) {
  const session = useSessionOptional()

  if (!session) {
    return <QuizImagePreview slide={slide} />
  }
  ...
  return isMaster
    ? <QuizImageMaster slide={slide} quiz={quiz} adapter={adapter} />
    : <QuizImagePlayer slide={slide} quiz={quiz} adapter={adapter} />
}

// ── Preview estático ──────────────────────────────────────────────────────

function QuizImagePreview({ slide }: { slide: Model }) {
  ...
  const choose = (i: number) => {
    ...
    setPicked(i)
    track('quiz_image_pick', { option_index: i, correct: i === slide.correctIndex, question_id: slide.questionId })
  }

  return (
    <div style={containerStyle}>
      <QuizImage slide={slide} />
      <p style={stemStyle}>{slide.stem}</p>
      <div style={{ display: 'grid', gap: 10, marginTop: 16 }}>
        {slide.options.map((opt, i) => {
          const isCorrect = picked !== null && i === slide.correctIndex
          const isWrong = picked === i && i !== slide.correctIndex
          return (
            <button
              key={i}
              onClick={() => choose(i)}
              ...
              style={{
                ...
                textAlign: 'left',
                width: '100%',
                borderColor: isCorrect ? '#16a34a' : isWrong ? '#dc2626' : picked === i ? 'var(--color-lab-accent, #534AB7)' : 'var(--color-lab-rule, #D8D5CB)',
                background: isCorrect ? 'rgba(22,163,74,0.08)' : isWrong ? 'rgba(220,38,38,0.07)' : picked === i ? 'rgba(83,74,183,0.07)' : 'var(--p21-surface, #FFF)',
              }}
            >
    ...
    <div style={containerStyle}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          ...
          {!isIdle && (
            <button onClick={() => adapter.resetQuiz(slide.questionId)} style={actionBtn}>
              resetar
            </button>
          )}
        </div>
      ...
      <div style={{ display: 'grid', gap: 10, marginTop: 16 }}>
        {slide.options.map((opt, i) => {
          ...
          const barColor = isClosed ? (isCorrect ? '#16a34a' : '#9ca3af') : 'var(--p21-blue, #2563EB)'
          ...
          return (
            <div key={i} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', border: `1.5px solid ${isClosed && isCorrect ? '#16a34a' : 'var(--color-lab-rule, #D8D5CB)'}`, background: 'var(--p21-surface, #FFF)' }}>
              {quiz.status !== 'idle' && (
                <div style={{ position: 'absolute', inset: 0, right: 'auto', width: `${pct}%`, background: barColor, opacity: 0.12, transition: 'width 0.4s ease' }} />
              )}
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center', padding: '10px 14px', gap: 10 }}>
                ...
                {isClosed && isCorrect && <span style={{ color: '#16a34a', fontWeight: 700 }}>✓</span>}
              </div>
            </div>
          )
        })}
          ...
          {slide.options.map((opt, i) => {
            ...
            return (
              <button
                ...
                style={{ ...optionBase, cursor: isOpen && !answered ? 'pointer' : 'default', borderColor, background: bg, textAlign: 'left', width: '100%', transition: 'border-color 0.2s, background 0.2s' }}
              >
                <span style={letterStyle}>{letters[i]}</span>
                <span style={{ flex: 1 }}>{opt}</span>
                {isCorrect && <span style={{ color: '#16a34a', fontWeight: 700 }}>✓</span>}
...
function QuizImage({ slide }: { slide: Model }) {
  return (
    <img
      src={slide.image}
      alt={slide.imageAlt ?? slide.stem}
      style={{
        maxWidth: '100%',
        maxHeight: 320,
        objectFit: 'contain',
        borderRadius: 8,
        marginBottom: 'var(--spacing-lab-3, 0.75rem)',
        display: 'block',
      }}
    />
  )
